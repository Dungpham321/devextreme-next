...
const dashboard = () => {
  ...
  //các cột của lưới
  const col = [
    { df: "EmployeeID", c: "Nhân viên ID", v: false, rq: true },
    { df: "FullName", c: "Họ tên" },
    { df: "Position", c: "Chức vụ" },
    { df: "BirthDate", c: "Ngày sinh", dt: 'date', f: 'dd/MM/y' },
    { df: "Address", c: "Địa chỉ", cp: 2 },
    { df: "HomePhone", c: "Số điện thoại", cp: 2 }
  ]
  
  const handleGetData = () => {
      const selectedRow =  gridRef.current?.instance();
  ...
  return (
    <div className="font-[family-name:var(--font-geist-sans)]">
      <Grid_custom
        ...
        ref={gridRef}
      />
       <button onClick={handleGetData}> đã chọn</button>
    </div>
  )
}
